...
export class MdFileProcessor {
  ...
  private partitionContent(content: string): string[] {
    const remove_header = content.match(/#{3,5}\s{0,1}Q{0,1}\.{0,1}\s{0,1}\d+(.|\n)*/g);
    ...
    challengeRawPartition = challengeRawPartition.filter((item) => item.split("\n").length > 4).map(trimNewline);
  ...
  public extractQuestionIndex(challengeRaw: string): number {
    ...
    const index_match = (question.match(/Q\d+/g) || [undefined])[0]?.replace("Q", "");
  ...
  public extractAnswers(challengeRaw: string): number[] {
    ...
    const answers = result?.map((item) => (item.match(/\[x\]/i) ? true : false));
    const answersIdx = answers?.map((bool, i) => (bool ? i : -1)).filter((i) => i !== -1);
    return answersIdx || [];
  }
  ...
  public extractChoices(challengeRaw: string): string[] {
    ...
    const cleanedLastChoiceDetail = choiceExplainArray[choiceExplainArray.length - 1].replace(explain || "", "");
  ...
  public extractExplanation(challengeRaw: string): string | undefined {
    ...
    const lastChoiceDetail = choiceAnswerExplainArray[choiceAnswerExplainArray.length - 1];
  ...
  public getChallenge(challengeRaw: string): Challenge {
    ...
    const question = this.extractQuestion(challengeRaw).replace(/Q\d+.\s{0,1}/g, "");
    ...
    const challenge = new Challenge(index, question, choices, answers, explanation);
